...
public class ProductRepository : IProductRepository
{
    ...
    public ICollection<Product> GetProductsForCategory(int categoryId)
    {
        ...
        return _db.Products.Where(p => p.CategoryId == categoryId).Include(p => p.Category).OrderBy(p => p.Name).ToList();
    }
    ...
    {
        IQueryable<Product> iQuery = _db.Products;
        query = query.ToLower().Trim();
        if (!string.IsNullOrEmpty(query))
        {
            iQuery = iQuery.Include(p => p.Category).Where(p => p.Name.ToLower().Trim().Contains(query) ||
            (p.Description != null && p.Description.Contains(query)));
        }

        return iQuery.OrderBy(p => p.Name).ToList();
    }
